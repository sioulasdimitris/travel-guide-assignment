...
@section('main')
    ...
    @php
        $post_id = $post_seo_title = $post_seo_description = $post_category = $post_thumb = '';
    @endphp
    
    @if(isset($post))
        @php
            $post_id                = $post['id'];
            $post_seo_title         = $post['seo_title'];
            $post_seo_description   = $post['seo_description'];
    ...
    <div class="row">
        <form action="{{route('admin_post_create')}}" enctype="multipart/form-data" method="post">
                ...
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="seo_title" name="seo_title" value="{{$post_seo_title}}">
                        </div>
                        <div class="form-group">
                            <label for="seo_description">Meta Description:</label>
                            <textarea class="form-control" id="seo_description" name="seo_description">{{$post_seo_description}}</textarea>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-4 col-sm-4 col-xs-12">
                ...
                @if((isset($post_type) && $post_type === \App\Models\Post::TYPE_BLOG) || (isset($post['type']) && $post['type'] === \App\Models\Post::TYPE_BLOG))
                    <div class="x_panel">
                        <div class="x_title">
                            <h2>Category</h2>
                            <div class="clearfix"></div>
                        ...
                        <div class="x_content">
                            @foreach($categories as $cat)
                                <div class="checkbox">
                                    <label>
                                        <input type="checkbox" class="flat" name="category[]" value="{{$cat->id}}" {{isChecked($cat->id, $post_category)}}> {{$cat->name}}
                                    </label>
                                </div>
                            @endforeach
                        </div>
                ...
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <label class="post_thumb" for="post_thumb">
                            <img id="preview_thumb" src="{{getImageUrl($post_thumb)}}" alt="post thumb">
